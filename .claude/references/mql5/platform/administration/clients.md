...
> Source: https://support.metaquotes.net/en/docs/mt5/platform/administration/clients
...
[MetaTrader 5](/en/docs/mt5)[Platform](/en/docs/mt5/platform)[Platform Setup](/en/docs/mt5/platform/administration)Clients

# Clients

The Client is a separate entity in the trading platform. This is the aggregate information on a trader, including [personal data](/en/docs/mt5/platform/administration/clients#personal), [documents](/en/docs/mt5/platform/administration/clients#documents) and [trading accounts](/en/docs/mt5/platform/administration/clients#accounts). The client record stores the history of all changes: every version made by any manager is available in the [Versions](/en/docs/mt5/platform/administration/clients#versions) section.
...
![Client management section](/en/docs/mt5/platform/img/clients.png "Client management section")

Before you start, request information on desired clients from the server. At the bottom of the section, specify the first name, last name, phone number, email or client ID, and then click Request. To receive information on all clients, specify the mask "\*".

<table class="attentable" cellspacing="0" cellpadding="8" border="1"><tbody><tr class="attentable"><td class="attentable"><ul><li class="p_tableatten"><span class="f_tableatten">All clients are stored separately for each <a href="/en/docs/mt5/platform/administration/admin_network/network_add_edit/network_trade_server" class="topiclink">trading server</a>. Client records are created on the same server, where the clients' trading accounts are exist (as well as the <a href="/en/docs/mt5/platform/administration/admin_groups/groups_settings#trade_server" class="topiclink">groups</a>, to which they belong). A <a href="/en/docs/mt5/platform/administration/admin_managers" class="topiclink">manager</a> can only view and manage clients, which exist on the same trade server, where the manager account is created.</span></li><li class="p_tableatten"><span class="f_tableatten">The manager account needs to have <a href="/en/docs/mt5/platform/administration/admin_managers#clients" class="topiclink">appropriate permissions</a> for accessing the Clients section.</span></li></ul></td></tr></tbody></table>

## Creating clients automatically and manually [#](clients#create)

All accounts registered in the platform via client terminals are [automatically linked to clients](/en/docs/mt5/platform/administration/clients#binding). If there is no client record for the newly created account, such a record is created automatically. All information from the account is copied to the client record. Minimum details are required for demo accounts: name, surname, company, email and phone number. More data can be requested during the registration of preliminary accounts via client terminals. Depending on platform settings, the following data may be requested:
...
If you enable the [extended account registration form](/en/docs/mt5/platform/administration/admin_accounts/account_allocation_groups#extended), all the above data will be automatically added to client records. Thus, you will not need to additionally request this information and add it manually to the database.

![Information specified during account registration is automatically added to a client record](/en/docs/mt5/platform/img/clients_info_copy.png "Information specified during account registration is automatically added to a client record")

To create a client manually, click "![New client](/en/docs/mt5/platform/img/add_button.png "New client")New Client" in the context menu of the client list or press "Ctrl+N".

![Creating a client](/en/docs/mt5/platform/img/clients_create.png "Creating a client")

Then specify required information in sections [Personal data](/en/docs/mt5/platform/administration/clients#personal), [Address](/en/docs/mt5/platform/administration/clients#address) and [Regulation](/en/docs/mt5/platform/administration/clients#regulation). Once you save the client record, document and trading account management sections will appear.

<table class="attentable" cellspacing="0" cellpadding="8" border="1"><tbody><tr class="attentable"><td class="attentable"><p class="p_tableatten"><span class="f_tableatten">When a <a href="/en/docs/mt5/platform/administration/admin_accounts/account_add" class="topiclink">trading account is created</a> via the Administrator terminal, an appropriate client record is not created automatically. To create it manually based on account data, open the account dialog and click "New Client" in the lower left corner. To create an account linked to a client record, use the <a href="/en/docs/mt5/platform/administration/clients#accounts" class="topiclink">"Trading accounts"</a> section in the appropriate client record.</span></p></td></tr></tbody></table>
...
Data are also copied to clients after each time [the account is changed](/en/docs/mt5/platform/administration/admin_accounts/account_edit). In this case, the information is only copied to the empty fields of the client. For example, when the manager adds an address to account data. If the address is not filled for the client to whom the account is bound, the data will be copied.

## Differentiation of access to clients for managers [#](clients#access)

A client record is available to the [manager account](/en/docs/mt5/platform/administration/admin_managers) if one of the following conditions is met:

-   The client is created by this manager
-   The client is explicitly [assigned](/en/docs/mt5/platform/administration/clients#assigned) to the manager
-   [The preferred trading group](/en/docs/mt5/platform/administration/clients#preferred_group), set for the client is available to the manager (the manager has [permissions for the group](/en/docs/mt5/platform/administration/admin_managers#groups))
-   Any of the [trading accounts bound to the client](/en/docs/mt5/platform/administration/clients#accounts) is available to the manager (the manager has [permissions for the group](/en/docs/mt5/platform/administration/admin_managers#groups) in which the account is located)
...
![General client information](/en/docs/mt5/platform/img/clients_general.png "General client information")
...
-   KYC status — the result of the client verification via a [KYC](/en/docs/mt5/platform/administration/clients#kyc) service.
-   Assigned manager — the name of the responsible employee. To find all the clients managed by a certain employee, enable the "Assigned manager" column in the client list then sort the list by this column. The list of managers available for selection is formed in accordance with the group settings of the manager who is viewing the client. If a [company](/en/docs/mt5/platform/administration/admin_groups/groups_settings#company) is specified the the current manager's group, only the managers from groups with the same company specified will be available to this manager. A manager without any company specified in the group, can access all managers on the server.
-   Preferred trading group — the field is used to [distribute clients between manager accounts](/en/docs/mt5/platform/administration/clients#access). If this field is empty, the client record will be available to managers regardless of their [account group permissions](/en/docs/mt5/platform/administration/admin_managers#groups). If the field is filled, the client will only be available to the managers having the specified group permissions.
...
![Selecting client data to export to file](/en/docs/mt5/platform/img/clients_export.png "Selecting client data to export to file")
...
![Client's Personal Data](/en/docs/mt5/platform/img/clients_personal.png "Client's Personal Data")
...
-   Language — the language spoken by the client. If the client record is created during the [registration of a preliminary account](/en/docs/mt5/platform/administration/clients#binding) through the client terminal, the language us set automatically based on the value specified in the registration form.
...
Brokers can check clients' personal data and [documents](/en/docs/mt5/platform/administration/clients#documents) in the automated mode using the trading platform integration with [KYC providers](/en/docs/mt5/platform/administration/integration/integration_kyc).

<table class="attentable" cellspacing="0" cellpadding="8" border="1"><tbody><tr class="attentable"><td class="attentable"><p class="p_tableatten"><span class="f_tableatten">The feature requires an appropriate <a href="/en/docs/mt5/platform/administration/integration/integration_kyc" class="topiclink">integration configuration</a>.</span></p></td></tr></tbody></table>

The trader [requests a live account](/en/docs/mt5/platform/administration/admin_accounts/preliminary_accounts) via the desktop or mobile terminal by filling out a form and uploading documents:

![Account opening form in the client terminal](/en/docs/mt5/platform/img/kyc_open_account_form.png "Account opening form in the client terminal")

A preliminary account with a zero balance is created for the client. This account along with the relevant data and documents is linked to the client record. The manager clicks the "KYC Check" button in the [client personal data](/en/docs/mt5/platform/administration/clients#personal) section and the platform automatically sends a request to the KYC system.

<table class="attentable" cellspacing="0" cellpadding="8" border="1"><tbody><tr class="attentable"><td class="attentable"><p class="p_tableatten"><span class="f_tableatten">The platform can be configured to enable <a href="/en/docs/mt5/platform/administration/integration/integration_kyc#auto" class="topiclink">automated sending</a> of client data to a KYC provider after the client opens a preliminary account via the client terminal. This allows the full automation of the onboarding process.</span></p></td></tr></tbody></table>

![Launching KYC check from the client personal data section](/en/docs/mt5/platform/img/kyc_check_start.png "Launching KYC check from the client personal data section")

The KYC system checks data and documents and returns the result, which the manager can view in the platform as a report:

![Client data check report](/en/docs/mt5/platform/img/kyc_report.png "Client data check report")

Based on the check result, the 'confirmed' or 'rejected' status is automatically assigned to the [client](/en/docs/mt5/platform/administration/clients#kyc_status) and to the documents uploaded to the relevant client record. Furthermore, data from the KYC system can supplement client records on the platform side. These include the details which the system can read from the provided documents: name, date of birth, document type, number and expiration date, among other data. Updated data is indicated in a separate report block.

![The document status is updated automatically according to the check result](/en/docs/mt5/platform/img/kyc_documents_status.png "The document status is updated automatically according to the check result")
...
![Client's address](/en/docs/mt5/platform/img/clients_address.png "Client's address")
...
This section is only available for [corporate clients](/en/docs/mt5/platform/administration/clients#client_type). Company registration data are specified here.

![Information about a corporate client](/en/docs/mt5/platform/img/clients_company.png "Information about a corporate client")
...
![Additional client information for regulators](/en/docs/mt5/platform/img/clients_regulationn.png "Additional client information for regulators")
...
![Client's documents](/en/docs/mt5/platform/img/clients_documents.png "Client's documents")

The list shows general information about documents: document ID, type, etc. To view more information about the document, double-click on its line in the list. To view the document, click![Download the document](/en/docs/mt5/platform/img/clients_documents_download.png "Download the document").
...
You can view the document in the Files tab and replace it if necessary. The tab shows the date of document addition, the name of the document author, the file name and size. To view the document file, click![Download the document](/en/docs/mt5/platform/img/clients_documents_download.png "Download the document"). If you have appropriate permissions, you can upload new document files by clicking "Attach documents".

![Viewing and editing a document](/en/docs/mt5/platform/img/clients_documents_view.png "Viewing and editing a document")

### Document versions

Any changes made to client documents are displayed on the Versions tab:

![Document versions](/en/docs/mt5/platform/img/clients_documents_versions.png "Document versions")
...
All messages from this section are also displayed in the general list of [client comments](/en/docs/mt5/platform/administration/clients#comments).

![Comments on client documents](/en/docs/mt5/platform/img/clients_documents_comments.png "Comments on client documents")
...
![Working correspondence in the client record](/en/docs/mt5/platform/img/clients_comments.png "Working correspondence in the client record")

All comments on [client documents](/en/docs/mt5/platform/administration/clients#documents) are also displayed in this section.
...
![Version storage of client data](/en/docs/mt5/platform/img/clients_versions.png "Version storage of client data")
...
Also, this section allows you to [open accounts](/en/docs/mt5/platform/administration/admin_accounts/account_add) for the client. To do this, click "New Account" in the context menu. The account created will be automatically connected to the client record. To edit an existing account, double-click on it in the list. After that, all account [parameters](/en/docs/mt5/platform/administration/admin_accounts/account_edit) will be shown in the right window part.
...
![Client's trading accounts](/en/docs/mt5/platform/img/clients_accounts.png "Client's trading accounts")

You may also use the context menu to export accounts to a file or to change the list view: show/hide columns and grid or enable/disable auto-size of columns.

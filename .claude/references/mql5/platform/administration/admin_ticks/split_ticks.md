...
> Source: https://support.metaquotes.net/en/docs/mt5/platform/administration/admin_ticks/split_ticks
...
[MetaTrader 5](/en/docs/mt5)[Platform](/en/docs/mt5/platform)[Platform Setup](/en/docs/mt5/platform/administration)[Bid/Ask/Last Ticks](/en/docs/mt5/platform/administration/admin_ticks)Split Stocks
...
-   Transforming the [minute](/en/docs/mt5/platform/administration/admin_charts/split_charts) and tick symbol history means proportional increase or decrease in prices, spreads and volumes.
...
Click "Split Stock" in the context menu of the "Bid/Ask/Last Ticks" section.

![Transforming tick data when splitting stocks](/en/docs/mt5/platform/img/split_ticks.png "Transforming tick data when splitting stocks")

Select a trading instrument and a time interval, and click Request. Ticks appear in the list.

Set the split settings:

-   Use "New Shares" and "Old Shares" to set stock split/consolidation ratio so that tick data can be transformed similarly.
...
-   If necessary, you can specify the time interval in which the split will be performed, using the "Split from" and "To" fields. In this case the operation will only be performed for tick data in the specified range, while the remaining data will not be affected. Such an operation may be needed for performing a backdated split. If you want to split the entire available symbol history, leave these parameters unchanged.

After implementing all the settings, click Calculate to see the preliminary split results. New Bid, Ask and Last prices for each tick are displayed in the table. Click Process to execute a split.
...
## Split Scanner [#](split_ticks#split_scanner)
...
Go to the "Bid/Ask/Last Ticks" section and select "![Split Scanner](/en/docs/mt5/platform/img/split_scanner_icon.png "Split Scanner")Split Scanner" in the context menu. Next, select a group of symbols and click "Scan". Check the received split points. The list will display:

...
Returns all asset classes (Forex, Indices, Commodities, etc.).

---

## Request — `ProtoAssetClassListReq` (payloadType `437`)
...
fields = c.request(
    PT["ASSET_CLASS_LIST_REQ"],
    PT["ASSET_CLASS_LIST_RES"],
    inner
)
...
## Response — `ProtoAssetClassListRes` (payloadType `438`)

### Response Fields

| Field | Type | Description |
|-------|------|-------------|
| `assetClass` | `ProtoAssetClass[]` | Asset class ID and name |
...
Asset classes group assets and symbols for display and margin purposes.

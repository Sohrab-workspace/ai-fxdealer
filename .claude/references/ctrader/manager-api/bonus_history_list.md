...
Returns bonus credit/debit history.

---

## Request — `ProtoBonusHistoryListReq` (payloadType `786`)
...
fields = c.request(
    PT["BONUS_HISTORY_LIST_REQ"],
    PT["BONUS_HISTORY_LIST_RES"],
    inner
)
...
## Response — `ProtoBonusHistoryListRes` (payloadType `787`)

### Response Fields

| Field | Type | Description |
|-------|------|-------------|
| `bonusDepositWithdraw` | `ProtoBonusDepositWithdraw[]` | Bonus operation records |
...
Bonus balances are separate from cash balance. isWithdrawable=false means non-withdrawable bonus.
